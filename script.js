const list = [
  ...
  function page (num){
    //content will be shown by this id
    document.getElementById('displaytable').innerHTML = '';
    // for creating the table with css
    let table = document.createElement('table')
    table.setAttribute('class', 'table table-sm table-striped table-hover')
    // for creating the tablehead with css
    let tablehead = document.createElement('thead')
    tablehead.setAttribute('class', 'thead-dark')
    // table row with table head for contents
    ...
    // for diaplaing the content at table
    tableRow.append(id,name,email);
    tablehead.append(tableRow);

    let tableBody = document.createElement('tbody')
    //loop for displaying the content depending upon the number of pages
    for (i = (num - 1 )*10; i < num * 10; i++){

        let tablebodyrow = document.createElement('tr')
        let idd = document.createElement('td')
        idd.innerHTML= list[i].id
        let Name = document.createElement('td')
        Name.innerHTML= list[i].name
        let Email = document.createElement('td')
        Email.innerHTML= list[i].email

        tablebodyrow.append(idd,Name,Email);
  ...
  //calling the funtion for first last prev next actions and directing it to that page number
    function first (){
